# 当前活跃任务 (中期记忆 — 每次session加载)

> 最后更新: 2026-07-07 23:59

## 🔴 进行中 → M2 待开发
### IELTS陪练助手v2.0 — M1 验收完成，剩余问题转入 M2
- **雅思** 口语/写作/听力/阅读全线陪练
- **管线1-IELST Part1**: ✅ 验收 70%
  - 已完成: 链式流式核心管线、Debug验收模块、评估报告系统、成本追踪、VAD音频采集修复
  - 待解决: **延时优化**（当前 7700-11787ms，需降到 3000-5000ms）
- **Debug 验收模块**: ✅ 验收 90%
  - 已完成: SQLite 持久化、实时监控、诊断引擎、管线1/管线2 双面板
  - 待解决: 管线2 debug 数据完整性
- **管线2-Qwen Omni**: ❌ 未通过验收
  - 已知问题: Voice 不支持(Cherry)、buffer too small、WebSocket frame 超限(>256KB)
  - 决定: 暂停管线2开发，转入 M2 一起规划

### M2 开发计划（Daryl 7/5 指令）
- **P0**: 管线1 延时降至 3000ms 以下
  - ASR: batch→streaming API（预期 4617→300ms）
  - TTS: 串行→并行调用（预期 5595→2000ms）
- **P1**: 管线2 Qwen Omni 修复
  - 音频分片发送（解决 256KB frame 限制）
  - Voice 参数修正
- **P2**: Part 2 长独白 / Part 3 深度讨论模式
- **P2**: 评估面板 UI 优化

### Loop Engineering（基建 · P0 规划中）
- **状态**: 🟡 需求澄清（Grill-me 进行中）
- **方向**: 决策矩阵延伸，短链→长链，P2/P3 自主执行
- **M1**: 需求澄清+详细计划 → 预计 7/7
- **M2**: 核心框架开发 → 预计 7/20
- **M3**: Agent集成+联调 → 预计 7/31
- **阻塞**: 等待 Daryl DM 互动澄清需求

## 🟢 已完成
### IELTS陪练助手v2.0 M1 交付清单
| 模块 | 状态 | 完成度 |
|------|------|--------|
| 管线1 链式流式核心 | ✅ | 100% |
| 配置管理 + API Key | ✅ | 100% |
| SQLite 会话持久化 | ✅ | 100% |
| HTML 评估报告 | ✅ | 100% |
| Debug 验收模块 | ✅ | 90% |
| VAD + 音频采集修复 | ✅ | 100% |
| 成本追踪 Dashboard | ✅ | 100% |
| OPC 看板接入 | ✅ | 100% |
| 管线2 Qwen Omni 对接 | ❌ | 30% |

### OPC Dashboard v1.6.1 设计（🟡 待 Daryl 审批）
- **状态**: 方案已交付，等 Daryl 拍板 3 个 P1 问题
- **产出**: `opc_dashboard/docs/DESIGN_v1.6.1.md` + 3 张配图
- **核心**: 5tab→4场景组、设计 token 化、交互链路收敛
- **待确认**: 导航重组/合规仪表板接入/active.md路径更正

### 视频分析交互Workflow 🎬
- **状态**: 🟡 standby（V3 Gemini 原生视频方案已验收，代码已归档）
- **里程碑**: v1.0 MVP → v1.1 商业数据+Tab → v1.2 Gemini V3 原生视频 ✅ (7/4)
- **下一阶段**: 内核驱动洗稿（待 Daryl 确认）
- **成本**: ~$0.0025/次分析
- **地址**: https://unwhispering-imani-digitately.ngrok-free.dev (端口8777)

## 🔵 待办
- ngrok 备用隧道安装
- 硬件语音助手(MCU)开发
- Agent自进化（待 Loop Engineering 框架就绪）
- OPC Dashboard crontab 看门狗 ✅（7/7 交付，`*/5 * * * *` 自动恢复tunnel+服务）
- 内核驱动洗稿MVP（待视频分析下一阶段确认）

## 📦 历史归档项目
- 抖音视频分析MVP（B站/douyin 下载器+分析，2026年4月）→ 已归档
- 语音转录测试(STT) → 已归档
- 投资者路演短语库 → 已归档
