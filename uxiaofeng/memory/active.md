...
> 最后更新: 2026-08-06 23:59（每日审计 · 工作区指针事故修复+M2试点开工）
...
### ⚠️ 8/6 工作区指针事故（已修复）
- 根因: 8/4 openclaw.json 重写时 agents.list 的 workspace 字段全被删除 → 新会话回退到空壳工作区导致失忆
- 修复: Kitty 已显式写死 4 个 agent 的 workspace；空壳记录合并；gateway 重启生效
- 行动项: [ ] 验证新会话读真工作区 [ ] 同步教训给 Kitty 入模式库
