...
| 阶段进度 | 核心链路 100%（7/25 ✅）；wf01-wf05 已冻结为 v4.0 基线（8/6）；前端 v4.0 交互版 60% 节点（8/7 核验，8/6-8/9 无前端开发投入，进度持平；8/9 ASR 需求第1条待 Daryl 确认） |
| 总进度 | 92%（8/7-8/9 无新增开发投入，持平） |
| 启动日期 | 2026-07-19 |
...
| v4.0 · M2 决策自主环试点（wf01-wf05 交互版） | 🟡 in-progress | 60% | 8/6 开工，8/7 核验 60%；8/7-8/9 无前端开发投入；8/9 ASR 交互需求开始流入（第1条待确认） |
...
|------|-----|
| 项目ID | ielts-tutor-v2 |
| 状态 | 🟡 verifying · v1.3.0 待 Daryl 验收；8/7 Daryl 实际使用练口语（隧道切 8767 + debug 拉起）→ 反馈 STT/ASR 不准 → 当晚修复并实测 100% 词级命中；8/8 修复 assessment 不出评分 Bug（DeepSeek Thinking 模式吞 max_tokens）+ 补数据回填 2f7ae680；8/9 隧道改 cloudflared 临时 URL（selling-human-pig-arise，ngrok 固定域名让给视频剪辑）；8/4 服务 LaunchAgent 持久化 localhost:8767 ACTIVE；8/5 OPC看板v1.6 同步 wf_ielts_tutor_v130 工作流 |
| 优先级 | P0 |
