...
> 最后编译: 2026-08-14 19:02 +07:00
> 负责人: Bryson | Agent ID: xiaofeng
> 下轮更新: 2026-08-15 07:00
...
| v4.0 · M2 决策自主环试点（wf01-wf05 交互版） | 🟡 in-progress | 进度修正（8/12 实测前端 0 行，停在线框冻结；原 60% 虚报） | 8/6 开工；8/7 核验 60%（虚报）；8/7-8/11 无前端开发投入；8/12 实测修正为 0 行，待重新排期；8/9 ASR 交互需求开始流入（第1条待确认，8/13 重发仍待确认，8/14 已 5 天） |
...
| 总进度 | 88%（8/8 assessment 修复后维持） |
| 启动日期 | 2026-06-08 |
| 预计交付 | v1.1.0 挂起中，v1.3.0 待验收 |
| 上周进展 | 7/25 Daryl 飞书调试发现 2 Bug 并修复 ✅：① DeepSeek 废弃 `deepseek-chat` → `deepseek-v4-pro` ② 跟读 JS null safe 兜底；Daryl 指令：自由对话切 `deepseek-v4-flash` + Debug 模式开启 ✅；8/4 服务 LaunchAgent 持久化（ai.openclaw.ielts，localhost:8767，health+dashboard 均 200）；8/5 OPC看板v1.6 同步 wf_ielts_tutor_v130 11 节点工作流（投资人展示准备）；8/7 晚 Daryl 要求练口语：ngrok 隧道 8768→8767 切换 + DEBUG_MODE=1 拉起 debug 模块（/debug/sessions 200）；Daryl 反馈 STT/ASR 识别不准 → 根因定位 VAD 过激（threshold=0.5+min_silence 600ms+pad 400ms 误删 30-43% 语音）+ beam=1 质量最低档 → 修复（VAD 0.35/900ms/700ms + beam 3）→ 实测 5.2s 短句 100% 词级命中 + 15s 长句 49/49 全对；8/8 Daryl 反馈自由对话无 assessment（session 2f7ae680 AUTO-EVAL score=0）→ 根因 deepseek-v4-flash 默认 Thinking 模式（reasoning 吃掉 2473B token，content 仅 101B 残缺 JSON）+ 三处调用未关 Thinking + server.py 从未调 add_transcript → 修复（三处加 `"thinking": {"type": "disabled"}` + 重启 LaunchAgent）→ 评估 content=854B 完整 JSON ✅；从 server.err.log 恢复 14 条用户文本补评估（overall=5.5, F5.5/V5.5/G5.0/P6.0）+ HTML 报告 + 补 transcripts；8/9 IELTS 隧道改 cloudflared 临时 URL（ngrok 免费版仅 1 个固定域名，固定域名让给视频剪辑）✅ 前端 200 + API 健康；8/13-8/14 无新使用反馈（8/14 Daryl 低谷对话给出雅思市场估值口径：中国细分市场 50-100亿/年，未实战验收） |
| 本周计划 | Daryl 实战验收 v1.3.0 + STT/ASR + assessment 修复效果（现入口为 cloudflared 临时 URL selling-human-pig-arise.trycloudflare.com，重启会变；如需固定 URL 需 Daryl 定夺域名归属）；如有口音个别词不准，评估 small.en→medium.en 或 IELTS_STT_MODEL 覆盖；transcripts 落库缺失待 Daryl 决定是否修复；Daryl 8/14 进入休息期，实战验收顺延 |
| 阻塞项 | ① iPhone 语音输入问题 → v1.1.0 挂起 ② ~~STT/ASR 识别不准~~ ✅ 8/7 已修复并实测（VAD+beam） ③ ~~assessment 不出评分~~ ✅ 8/8 已修复（thinking disabled + 补数据） ④ 等待 Daryl 实战验收 v1.3.0（8/7 已开始使用） ⑤ transcripts 从未落库（server.py 无 add_transcript 调用，待 Daryl 决定） |
...
| v1.3.0 · 深度对话+词汇+Dashboard | 🟡 verifying | 100% | 7/22（开发完毕）；8/7 STT/ASR 修复；8/8 assessment 修复（thinking disabled）后进入实战验证；8/13-8/14 仍待 Daryl 实战验收（无新反馈，Daryl 进入休息期） |
...
> *8/7 起 IELTS 恢复使用（隧道切换+STT 修复），相关 token 消耗并入 xiaofeng 8月合计（balance ledger 权威源；8/11 23:45 全量 cost-check：今日 $0.07 / 本月 $20.36 / 累计 $146.15）。
