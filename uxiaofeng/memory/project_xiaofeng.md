...
| 状态 | 🟡 active · v3.0 重构完成（7/25），7/31 Daryl 开始逐模块发送需求→01-素材扫描可行性评估已回复（~3.5h全部可行）；8/1 隧道刷新为 synthesis-ent-hawaii-booth |
| 优先级 | P0 |
| 当前阶段 | v3.0 重构 · 模块化交互需求逐个接收中（7/31 已收到 01-素材扫描） |
| 阶段进度 | 100%（7/25 核心链路走通 ✅）；交互需求第一模块已接收（7/31），等待Daryl确认2个前置问题（已等2天） |
| 总进度 | 90% |
| 启动日期 | 2026-07-19 |
...
| 本周计划 | 等待 Daryl 确认素材扫描 2 个前置问题（已等2天）；后续模块需求陆续接收中；BGM 替换等待 Daryl 提供《I NEED YOU》文件；隧道 maintenance 按需刷新 |
...
|------|-----|
| 项目ID | rewrite-mvp |
| 状态 | 🟡 standby · 需求收集模式（已 15 天无需求输入，自 7/18 v1.2.0 发布起） |
| 优先级 | P1 |
...
| 本周计划 | 等待 Daryl 发送具体需求；已 15 天无输入 |
| 阻塞项 | Daryl 尚未发送具体需求 |
...
|------|-----|
| 项目ID | ielts-tutor-v2 |
| 状态 | 🟡 verifying · v1.3.0 待 Daryl 晚间验收，7/25 Daryl 调试发现2Bug已修复（已等 8 天，7/30-8/1 无晚间练习） |
| 优先级 | P0 |
