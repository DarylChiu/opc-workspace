...
| 状态 | 🟡 active · v3.0 重构完成（7/25），7/31 Daryl 开始逐模块发送需求→01-素材扫描可行性评估已回复（~3.5h全部可行） |
| 优先级 | P0 |
| 当前阶段 | v3.0 重构 · 模块化交互需求逐个接收中（7/31 已收到 01-素材扫描） |
| 阶段进度 | 100%（7/25 核心链路走通 ✅）；交互需求第一模块已接收 |
| 总进度 | 90% |
| 启动日期 | 2026-07-19 |
| 预计交付 | 2026-08 |
| 上周进展 | 7/28 飞书隧道修复 ✅；7/30 BGM节点v2 完成：歌曲卡片/音频播放器/乐句播放+和弦弹窗/ffmpeg乐句连接 ✅；7/31 隧道刷新（degree-human-intro-airports.trycloudflare.com）→ Daryl 飞书发送 01-素材扫描模块 4 项改造需求 → 小风回复可行性评估（~3.5h，全部可行：目录浏览器/日期后缀/卡片视频预览/选取切换），等待 Daryl 确认 2 个前置问题 |
| 本周计划 | 等待 Daryl 确认素材扫描前置问题后一次性迭代；后续模块需求陆续接收中；BGM 替换等待 Daryl 提供《I NEED YOU》文件 |
| 阻塞项 | ① 等待 Daryl 确认素材扫描 2 个前置问题（根目录范围+日期格式） ② DJI Mic 3 音频同步仅框架+模拟 ③ BGM 替换等待 Daryl 提供 QQ 音乐文件 ④ Toshiba HDD exFAT 卡死，等 Daryl 物理重新插拔 |
...
|------|-----|
| 项目ID | rewrite-mvp |
| 状态 | 🟡 standby · 需求收集模式（已 14 天无需求输入，自 7/18 v1.2.0 发布起） |
| 优先级 | P1 |
...
| 启动日期 | 2026-07-15 |
| 预计交付 | TBD |
| 上周进展 | 无变化，维持 v1.2.0（内核卡 M1-M3 完整交付，6 个 commit）；7/30-31 无新需求 |
| 本周计划 | 等待 Daryl 发送具体需求；已 14 天无输入 |
| 阻塞项 | Daryl 尚未发送具体需求 |
...
|------|-----|
| 项目ID | ielts-tutor-v2 |
| 状态 | 🟡 verifying · v1.3.0 待 Daryl 晚间验收，7/25 Daryl 调试发现2Bug已修复（已等 7 天，7/30-31 无晚间练习） |
| 优先级 | P0 |
...
| 上周进展 | 7/24 数字资产恢复 ✅；7/25 Daryl 飞书调试发现 2 Bug 并修复：① DeepSeek 废弃 `deepseek-chat` → `deepseek-v4-pro` ② 跟读 JS null safe 兜底；Daryl 指令：自由对话切 `deepseek-v4-flash` + Debug 模式开启 ✅；7/30-31 无晚间验收活动 |
| 本周计划 | Daryl 晚间验收 v1.3.0；已等 7 天无晚间练习，后续待定 |
