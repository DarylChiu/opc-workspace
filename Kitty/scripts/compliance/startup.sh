#!/bin/bash
# L0: 启动验证 — 合规系统健康检查 (Kitty workspace)
...
echo "🔍 L0 合规启动验证 | $TIMESTAMP (Kitty)"
echo "================================"
...
check_file "memory/_working_active_tasks.md" "active_tasks"
check_file "memory/_working_recent_conversations.md" "recent_conversations"
check_file "memory/workflow-rules.md" "workflow-rules"
...
if [ -f "$TODAY_FILE" ]; then
    ...
    ISSUES+=("今日日记 $TODAY.md 缺失")
    echo "  ⚠️ 今日日记缺失"
fi

# 3. active_tasks 新鲜度检查
if [ -f "memory/_working_active_tasks.md" ]; then
    LAST_MODIFIED=$(stat -f "%m" "memory/_working_active_tasks.md" 2>/dev/null || stat -c "%Y" "memory/_working_active_tasks.md" 2>/dev/null)
    NOW=$(date +%s)
    AGE_HOURS=$(( (NOW - LAST_MODIFIED) / 3600 ))
    if [ "$AGE_HOURS" -gt 48 ]; then
        ISSUES+=("active_tasks 超过48小时未更新（${AGE_HOURS}h）")
        echo "  ⚠️ active_tasks 老旧 — 最后更新 ${AGE_HOURS} 小时前"
    else
        PASSES+=("active_tasks 新鲜度正常（${AGE_HOURS}h）")
        echo "  ✅ active_tasks 新鲜度正常"
    fi
fi
...
cat > "$STATUS_FILE" << EOF
{
  ...
  "pass_list": $(printf '%s\n' "${PASSES[@]}" | python3 -c "import sys,json; print(json.dumps([l.strip() for l in sys.stdin if l.strip()]))" 2>/dev/null || echo "[]")
}
EOF
