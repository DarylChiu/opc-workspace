...
4. Read `memory/_working_active_tasks.md` — current tasks
...
#### L4 · 收尾自检（每次回复涉及实质工作后）
自问 3 个问题：
1. **产生了新任务/新决策/状态变更？** → 更新 `_working_active_tasks.md`
...
1. **Update `_working_active_tasks.md`** — remove completed items, add new tasks, update statuses
...
- **Working Memory** (`memory/_working_*.md`) — loaded every session, <50 lines each
  - `_working_active_tasks.md` — what you're doing right now
