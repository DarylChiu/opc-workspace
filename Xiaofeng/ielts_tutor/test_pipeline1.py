#!/usr/bin/env python3
"""IELTS v0.9.1 Pipeline 1 — 5 mode stress test"""
...
async def test_mode(mode, client, results):
    ...
    for rnd in range(ROUNDS + WARMUP):
        ...
        stat = {"r": rnd, "w": rnd < WARMUP, "stt": round(stt), "llm": round(llm),
                "tot": round(total), "txt": transcript[:50], "ch": chunks}
        stats.append(stat)
        if not stat["w"]:
            icon = "G" if total<5000 else "Y" if total<8000 else "R"
            print(f"  [{icon}] R{rnd} STT:{stt:.0f} LLM:{llm:.0f} TOT:{total:.0f}ms | {transcript[:40]}")

    await ws.close()
    real = [r for r in stats if not r["w"]]
    a = {k: round(sum(r[k] for r in real)/len(real)) for k in ["stt","llm","tot"]} if real else {"stt":0,"llm":0,"tot":0}
    print(f"  AVG: STT:{a['stt']} LLM:{a['llm']} TOTAL:{a['tot']}ms")
...
async def main():
    ...
    tt = time.time()-t0
    print(f"\n{'='*50}\nFINAL REPORT\n{'='*50}")
    print(f"| {'Mode':<20} | {'Rds':>3} | {'STT':>7} | {'LLM':>7} | {'TOTAL':>7} |")
    print(f"|{'-'*22}|{'-'*5}|{'-'*9}|{'-'*9}|{'-'*9}|")
    all_t = []
    for r in results:
        a = r["avg"]; n = sum(1 for x in r["rounds"] if not x["w"])
        all_t.append(a["tot"])
        print(f"| {r['mode']:<20} | {n:3d} | {a['stt']:5d}ms | {a['llm']:5d}ms | {a['tot']:5d}ms |")

    if all_t:
        ga = sum(all_t)/len(all_t)
        g = "A+" if ga<4000 else "A" if ga<5000 else "B" if ga<7000 else "C"
        print(f"\nGrand Avg: {ga:.0f}ms | Grade: {g} | Time: {tt:.0f}s")

    os.makedirs("test_reports", exist_ok=True)
    with open("test_reports/pipeline1_20260707.json", "w") as f:
        json.dump({"v":"0.9.1","results":results,"grand_avg_ms":round(ga),"time_s":round(tt)}, f, indent=2)
    print("Report: test_reports/pipeline1_20260707.json")
