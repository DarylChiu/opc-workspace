...
class QwenOmniClient:
    ...
    def __init__(self, api_key: str, mode: str = "ielts_part1",
                 voice: str = "Tina", language: str = "en-US",
                 model: str = None):
    ...
    MAX_CHUNK_BYTES = 200 * 1024  # Stay under 256KB WebSocket frame limit

    async def send_audio(self, pcm_chunk: bytes):
        """Send audio in sub-256KB chunks to respect Qwen's frame limit"""
        if not self.ws: return
        for i in range(0, len(pcm_chunk), self.MAX_CHUNK_BYTES):
            piece = pcm_chunk[i:i + self.MAX_CHUNK_BYTES]
