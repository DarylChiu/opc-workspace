    ...
    try:
        ...
        for phase in range(2):
            ...
            if is_douyin:
                ...
                bili_out = str(od / "bili_video")
                for retry in range(3):
                    if retry > 0: print(f"[VIDEO] you-get 重试 {retry}/2..."); time.sleep(3)
                    r = subprocess.run(["you-get","-o",od,"-O","bili_video",
                        "--format=dash-flv480-AVC",url],
                    ...
                    if cand:
                        ...
                        print(f"[VIDEO] you-get 尝试 {retry+1}: 未找到输出文件")
                else:
                    # 3次失败 → 尝试更高码率
                    print("[VIDEO] you-get 480p失败, 尝试360p")
                    r = subprocess.run(["you-get","-o",od,"-O","bili_video",
                        "--format=dash-flv360-AVC",url],
                        capture_output=True, text=True, timeout=120, cwd=str(od))
            else:
...
def _run_transcribe(task_id: str, url: str):
    ...
    try:
        ...
        if not t and vl and os.path.exists(vl):
            ...
            _pending_tasks[task_id] = {"status": "processing", "stage": "gemini"}
            try:
        ...
        _pending_tasks[task_id] = {"status": "processing", "stage": "deepseek"}
