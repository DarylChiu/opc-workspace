...
## ✅ 定案关闭（2026-09-17 10:25）

**Daryl 经 09-16 全天监控确认：根因 = 旧 DeepSeek API key 泄漏；使用新 key 后无成本失控 → 事故关闭，团队开发回归正轨。**

- 余额轨迹: ¥30.78（9/15 09:02 切换基线）→ ¥27.57（9/16 08:50）→ ¥23.01（9/17 10:25，本机探活）—— 平缓下滑，无凌晨跳变，无 pro 档异常请求。
- 旧 key `sk-b1715…c4be`: 401 已失效 ✓ / 新 key `sk-9638…35cf`: 200 ✓
- 结论：**「旧 key 仍被外部持有使用」得到验证**（三次事故同一指纹 + 换 key 后指纹消失）。
- 保持不启用：Sentinel、余额告警（Daryl 9/15 决定，不再提议）。

*记录：忧郁小猫 | 2026-09-15 建档 / 2026-09-17 关闭*
