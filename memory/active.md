...
### 8. IELTS陪练重构 🔄 → v2.0 开发中
- **状态**: 市场调研完成，双管线架构确定，Phase 0 基础骨架搭建中
- **架构**: 管线1(链式流式:Google STT→DeepSeek→Google TTS) + 管线2(Qwen-Omni 语音→语音)
- **决策**: 精度优先场景(IELTS模考/商业英语)走管线1，流畅优先场景(自由对话/发音)走管线2
- **月成本**: ~$5-10
- **产物**: ielts_tutor/WORKFLOW.md + 已有资产整合完成
- **开发阶段**: Phase 0 基础骨架 → Phase 1 管线1重构 → Phase 2 管线2接入
