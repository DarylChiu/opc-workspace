...
### 🆕 模型路由对齐（P1 待报 Daryl，8/21 发现）
- 8/21 OPC 群聊「模型成本/路由」大讨论中，Self 发现 runtime 实挂 deepseek-v4-pro（全队最贵档），但 AGENTS.md 路由写「sonnet-4.5 中型 / flash 轻量」——配置与规则不符
- 改法（与 Balance 一致）：轻量检索/格式化 → flash，深度研究 → sonnet，v4-pro 仅留跨域推导任务
- 状态：整理方案后报 Daryl 确认再动（涉及 P1 配置改动，不擅自切模型）

### ⚠️ 待办：清理旧 gateway cron 1799ac76（连续 15 天，已升级处理）
- 8/8—8/22 十五次审计均确认该 cron 仍启用（8/23 00:02 第十五次触发），与 main consolidated 23:59 双通道并存
- 已多次提醒 Kitty 删除，尚未处理；8/11 起按约定升级处理（8/23 继续观察等待，必要时上报 Daryl 裁决）
...
- 数据源: Balance 8/21 23:45 全量扫描（balance-jsonl-full-scan，cost_daily.json 权威源 $164.77）
